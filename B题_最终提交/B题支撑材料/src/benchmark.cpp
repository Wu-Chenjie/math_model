...
int main(int argc,char**argv){
 ...
 if(count<1||start<1||first<0||last<first||last>86||stress<0||stress>5)return 2;
 std::ofstream f(argv[1]);if(!f)return 2;
 f<<"problem,seed,method,stress,n,cleared,time_s,avg_s,move_m,measures,switches,miss,fallbacks,dp_calls,dp_actions,dp_fallbacks,dp_expanded,dp_budget_hits,aux_calls,aux_positive,joint_steps,runtime_s,certificate,shortcuts,shortcut_saved_m,residual_calls,residual_replaced,residual_retired,rollout_calls,rollout_replaced\n";
