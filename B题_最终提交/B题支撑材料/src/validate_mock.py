"""Independent nonofficial public-protocol Python simulator kernel + real HTTP tests of C++ solver."""
import argparse
import json
import sys
import threading
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--count',type=int,default=30);p.add_argument('--early',type=float,default=35);p.add_argument('--planner-depth',type=int,choices=[0,1,2,3],default=2);p.add_argument('--strategy',choices=['geometric','joint','previous','stage1','stage2','baseline'],default='joint')
    a=p.parse_args();rows=[]
    for problem in [3,4]:
    ...
    print(f'validated {len(rows)} nonofficial simulator cases (including 6 HTTP runs)')
