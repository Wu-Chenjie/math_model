...
int main(int argc,char**argv){
 ...
 o.routing=variant==3?1:variant==4?2:0;
 if(argc>12){o.shifted=true;o.shiftA=atof(argv[11]);o.shiftB=atof(argv[12]);o.routing=atoi(argv[10]);}
 if(argc>13)o.includeOrigin=atoi(argv[13])!=0;
 if(argc>14)o.opportunistic=atoi(argv[14])!=0;
 for(int p:{3,4})for(int seed=start;seed<start+count;++seed){
  std::string tr=argc>9&&std::string(argv[9])!="-"?std::string(argv[9])+"_p"+std::to_string(p)+"_"+std::to_string(seed)+".csv":"";
