...
def day_socs(c_days, d_days):
    """逐日真实(日初,日末)储电量：从6000起按 0.9c−d 逐日结转，
    与执行仿真/validate 的SOC链完全一致（P4两日滚动的日初≠6000）。"""
    s0 = 6000.0
    out = []
...
def _rebuild_charge_sheet(wb, c_days, d_days, socs, sheet_name='充放电量'):
    """重建充放电量sheet：表头+334天×6行；0:00/24:00储电量用逐日真实SOC。"""
    ...
    for i in range(334):
        ...
        ws.cell(row=base, column=6, value=round(float(s0), 2))
        ws.cell(row=base + 1, column=5, value='24:00')
        ws.cell(row=base + 1, column=6, value=round(float(s1), 2))
...
def gen_result2(npz, fname, price_mode='fixed'):
    ...
    _rebuild_charge_sheet(wb, c, d, day_socs(c, d))
