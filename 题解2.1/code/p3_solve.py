...
# 方案A结果单独存档（论文基线，需可溯源）
daysA = res['A_fc0_noAdj'][0]
np.savez_compressed(r'D:/shumo/shumoC/results/p3_A.npz',
                    p=np.array([x['p'] for x in daysA]),
                    c=np.array([x['c'] for x in daysA]),
                    dd=np.array([x['dd'] for x in daysA]),
                    e=np.array([x['e'] for x in daysA]),
                    plan_p=np.array([x['plan_p'] for x in daysA]))
print('saved results/p3_A.npz')
