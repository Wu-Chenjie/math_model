...
inline Choice plan(const geo::Poly&p,geo::Pt now,geo::Pt first,double angle,const std::vector<Event>&history,int problem,int depth,bool tuned,int limit,bool retreat=false,int samples=32,bool clear_neighborhood=false,int terminal_mode=0,int refinement=0,int bearing_bin=2,int robust_mode=0){
 ...
 if(robust_mode&&choice.available){
  auto old=geo::mec(p);
  int positive=0;for(auto e:history)positive+=e.kind==2;
  if(old.r>35&&(robust_mode!=2||positive==1)){
   auto fresh=robust_geometry::candidate(p,now,first,angle,history,problem,robust_mode==2?2:1);
   if(fresh.valid){
    double oldrho=choice.measure&&robust_geometry::reception(p,choice.point,history,problem)?robust_geometry::worst_radius(p,choice.point,128).upper:old.r;
    double extra=std::max(0.,geo::dist(now,fresh.q)-geo::dist(now,choice.point))/5+5;
    // Time-scaled diameter reduction is a selection heuristic, not a theorem
    // about total time. The held-out paired test is the promotion gate.
    bool accept=robust_mode==2?(fresh.rho<=35&&geo::dist(now,fresh.q)+25<geo::dist(now,choice.point)):(.4*(oldrho-fresh.rho)>extra&&fresh.rho<old.r*.85);
    if(accept){choice.point=fresh.q;choice.measure=true;choice.alternatives={{fresh.q,true,choice.value}};}
   }
  }
 }
 return choice;
