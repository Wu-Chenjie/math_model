...
checks=[{'name':n,'status':'pass','evidence':e} for n,e in [
 ...
 ('coverage_and_completion','All included local runs clear their actual source count; official outcomes remain unobserved.'),
...
m=get('modeling-manifest.json');m['project'].update(stage='delivery',status='limited')
...
m['models']['selected']=families[0][0];m['models']['selection_rationale']={'objective_fit':'直接比较总任务秒数；不以FIM或半径代理冒充最终目标','constraint_fit':'发现与光学覆盖保留几何证书','data_fit':'预冻结独立本地场景，开发不合并','baseline':'在同一编译器环境重跑56和既有78','risks':['Q4候选84对78收益区间跨零','官方正式结果缺失','一般浮点裁剪未形式验证'],'fallback_trigger':'预声明95%均值节省区间不全为正则不推广84；默认保留78'}
...
m['pending']=[{'id':'official-six-logs','status':'pending','reason':'Q3/Q4各三次官方正式成绩与原始加密日志不可得；当前仅交付有复核的本地研究稿。'}]
m['violations']=[]
...
print(json.dumps({'status':'LOCAL_AUDIT_PASS_OFFICIAL_PENDING','csv_files':len(record),'independent_runs':sum(z['rows'] for z in record if z['group']!='development'),'independent_environments':len(unique),'registered_results':len(entries)},ensure_ascii=False))
