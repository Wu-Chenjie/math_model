...
out={
 ...
 'constants':{'speed_m_s':v,'clear_radius_m':r,'successful_clear_s':5,
              'switch_s':1,'bearing_s':5,'failed_optical_s':3,'sources_min':10,'sources_max':16},
 'example':{'source_counts':[n0,n1],'distance_m':d,'added_route_m':d-r,
            'total_time_s':[float(t0),float(t1)],'mean_time_s':[float(mu0),float(mu1)],
            'mean_time_exact':[str(mu0),str(mu1)],'marginal_identity_exact':True},
 'input_statement_sha256':hashlib.sha256((R.parents[1]/'inputs/B题.pdf').read_bytes()).hexdigest(),
 'source_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
 'runtime_s':time.perf_counter()-start,
 'limitations':['No new cross-policy empirical validation.','No independent mathematical review.']}
(R/'artifacts/algebra-check.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n')
