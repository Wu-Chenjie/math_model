...
def run(cmd,cwd=R):
 begin=time.time();out=subprocess.run(cmd,cwd=cwd,capture_output=True,text=True);rec={'command':cmd,'cwd':str(cwd.relative_to(R)) if cwd.is_relative_to(R) else str(cwd),'exit_code':out.returncode,'runtime_s':time.time()-begin,'stdout':out.stdout,'stderr':out.stderr};records.append(rec)
 (R/'artifacts/reproduction-log.json').write_text(json.dumps(records,ensure_ascii=False,indent=2))
...
if a.paper:
 run([sys.executable,'src/build_paper_assets.py']);run([sys.executable,'src/write_solution.py'])
 for _ in range(2):run(['xelatex','-interaction=nonstopmode','-halt-on-error','main.tex'],R/'paper')
 run(['pandoc','B题几何策略优化_题解.md','--pdf-engine=xelatex','-V','documentclass=ctexart','-V','mainfont=Songti SC','-V','CJKmainfont=Songti SC','-V','geometry:margin=2.5cm','-V','papersize:a4','-V','fontsize=11pt','-o','B题几何策略优化_题解.pdf'])
 run([os.environ.get('PDF_PYTHON',sys.executable),'src/finalize_pdfs.py'])
print('Frozen results reproduced; geometric=84 experimental, joint=78 recommended.')
