...
int main(int argc,char**argv){try{int prob=argc>1?atoi(argv[1]):3;Options o=configuration(2);if(argc>2)o.early=atof(argv[2]);PipeSensor s;Policy bot(s,prob,o);bot.run();std::cout<<"exit "<<bot.cleared<<" "<<bot.certificate<<std::endl;}catch(const std::exception&e){std::cerr<<e.what()<<"\n";return 1;}}
