"""Exact rational sufficient certificate for all positions and hemispheres.
...
triangle lies in the convex hull of sites within 1000 of all leaf vertices.
...
def split(triangle):
    ...
    return (a,m,c),(m,b,c)
def certify(points,n=32,maxdepth=24,reception=F(1000)):
    reception=F(reception);assert 0<reception<=1000
    ...
    for root in range(len(outer)):
        ...
        while stack:
            ...
            if all(contains(hp,v) for v in tri):
    ...
    return dict(status='CERTIFIED' if not unresolved else 'UNKNOWN',points=[[str(x) for x in p] for p in points],
        radius=1800,reception=str(reception),tangent_quadrant_subdivisions=n,maxdepth=maxdepth,
...
def verify(cert):
    ...
    reception=F(cert['reception']);assert cert['radius']==1800 and 0<reception<=1000
    ...
    for root in range(len(outer)):
        ...
        while stack:
            ...
            if key in records:
                ...
                assert all(contains([points[i] for i in ids],v) for v in tri)
                used.add(key)
            else:
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('input');parser.add_argument('output',nargs='?');parser.add_argument('--n',type=int,default=32);parser.add_argument('--maxdepth',type=int,default=24);parser.add_argument('--reception',default='1000');parser.add_argument('--verify',action='store_true');a=parser.parse_args()
    ...
    if a.verify:print('VERIFIED leaves',verify(raw))
        ...
        cert=certify([tuple(map(F,p)) for p in points],a.n,a.maxdepth,F(a.reception))
