"""Wait for this run's audit, rebuild the manuscript, and render all pages for review."""
from pathlib import Path
import json,subprocess,time,sys,platform,shutil,hashlib
import numpy as np
...
while True:
    a=S/'capacity-audit.json';b=S/'batch-status.json'
    if a.exists() and b.exists() and read(a).get('status')=='PASS' and len(read(b))==8:break
    time.sleep(30)
print('AUDIT_READY',flush=True)
shutil.copy2(P/'main.tex',P/'revised-main.tex')
start=time.perf_counter()
with (S/'reproduce.log').open('w') as log:
    r=subprocess.run(['sh','reproduce.sh','check'],cwd=P,stdout=log,stderr=subprocess.STDOUT)
record={'command':'sh reproduce.sh check','exit_code':r.returncode,'runtime_seconds':time.perf_counter()-start,'environment':{'python':sys.version,'platform':platform.platform(),'numpy':np.__version__},'scope':'Independently recheck12 saved capacity trajectories, rebuild capacity/inventory figures and tables, reproduce prior paper checks and compile current PDF. Eight new annual solves separately recorded in batch-status.json.'}
dump(S/'execution.json',record);assert r.returncode==0,'See reproduce.log'
subprocess.run([sys.executable,str(S/'freeze_storage.py')],cwd=P,check=True)
print('PDF_COMPILED',flush=True)
