...
lines+=['','四机制各三组完整334日JSON/NPZ均已找到并重算。比较统一首末6000、beta=1、legacy场景和上层、M0反馈及其余参数；固定48与72只改变配置horizon_hours，旧午夜与固定48只改变horizon_mode。它们是配置因素对照，时域变化还同时改变合法成熟历史池和末值取价区间，不是固定所有诱导量的纯物理前瞻效应。该表不使用beta=.75的远端统一候选，避免把融合收益混入时域对照。正负日数、完整日账单、配置差及哈希见JSON。', '', '## 开发期配对预测误差矩','',
...
lines+=['','beta*=clip((B−C)/D,0,1)，D>0。MSE与MAE不同，更与受不对称紧急费和库存制约的闭环账单不同；不能把此样本beta替代现有.75配置。完整候选权重的MSE、MAE、偏差与相关系数保存在JSON。','',
