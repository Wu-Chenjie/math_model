...
def main():
 ...
 claims['provenance']={str(p.relative_to(R.parent)):hashlib.sha256(p.read_bytes()).hexdigest() for p in list((R/'artifacts/global-terminal').glob('*.npz'))+list((O/'artifacts/ablation_fixed').glob('*.npz'))}
