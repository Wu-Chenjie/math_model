...
C=json.loads((O/'artifacts/result-claims.json').read_text());H=json.loads((O/'artifacts/handoff-tables.json').read_text());K=list(C['models']);labels=['问题二','问题三','问题四日前','问题四日内'];colors=['#2b8c91','#cb655d','#7683b4','#d79a57']
...
def table(name,caption,head,rows,spec=None):
 ...
 (O/f'tables/{name}.tex').write_text('\n'.join(lines)+'\n')
