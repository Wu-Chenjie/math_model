...
const style=(s,rows,cols)=>{
 const r=s.getRangeByIndexes(0,0,rows,cols);r.format.font={name:'Arial',size:11};r.format.rowHeight=20;r.format.verticalAlignment='center';
...
for(const [file,key] of pairs){
 if(process.argv[2] && process.argv[2]!==file) continue;
 ...
 if(!key){
  ...
  const bs=wb.worksheets.getItem('充放电量');bs.getUsedRange().unmerge();bs.getUsedRange().clear({applyTo:'all'});
 ...
 ns.getRange(`A1:B${payload.notes.length}`).format.rowHeight=32;
 ns.getRange('A1:B1').format.rowHeight=22;
 ...
 // Reimport the saved file so previews use its final unmerged layout.
 const saved=await SpreadsheetFile.importXlsx(await FileBlob.load(`${out}/${file}.xlsx`));
 for(const s of saved.worksheets.items){
  const range=s.name==='口径说明'?`A1:B${payload.notes.length}`:s.name==='计划购电量'&& !key?'A1:B8':s.name==='紧急购电量'?'A1:C8':s.name==='费用汇总'?'A1:J8':'A1:F8';
  const blob=await saved.render({sheetName:s.name,range,scale:1.3,format:'png'});
  await fs.writeFile(`artifacts/previews/${file}-${s.name}.png`,new Uint8Array(await blob.arrayBuffer()));
  if(s.name==='调整购电量'){
   const revised=await saved.render({sheetName:s.name,range:'AL1:AS8',scale:1.3,format:'png'});
   await fs.writeFile(`artifacts/previews/${file}-${s.name}-6点.png`,new Uint8Array(await revised.arrayBuffer()));
  }
 }
