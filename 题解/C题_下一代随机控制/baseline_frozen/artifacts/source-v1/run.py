...
def tail_file(kind,cutoff):return ROOT/f'artifacts/tails/{kind}_{cutoff}.json'

def train_job(job):
    kind,cutoff,iterations,steps=job;official,variable=KINDS[kind]
    data=dict(np.load(ROOT/'artifacts/data.npz'));res=train_tail(data,cutoff,variable,official,K=steps,iterations=iterations)
    ...
    dump(tail_file(kind,cutoff),res)
    return {'kind':kind,'cutoff':cutoff,'runtime_seconds':res['runtime_seconds'],'evaluation':res['evaluation']}

def simulate(data,selection,days,kind,candidate,count=7,horizon_days=2,tail_scale=1.,grid=321,final=None):
    ...
    E=6000.;logs=[];decisions=[];lpv=gap=0.;start=time.perf_counter();dp=None;cache={}
    ...
    for i,day in enumerate(days):
        ...
        for t in range(144):
            ...
            if t%36==0:
                ...
                active=t;ema=0.
                decisions.append({**meta,'date':str(data['dates'][day]),'release':t//36,'inventory':E,'tail_price':lam,'tail_model_cutoff':cutoff if cuts else None,'tail_cut_count':len(cuts) if cuts else 0})
            j=t-active
            if lower=='affine':
    ...
    metrics={'kind':kind,'candidate':candidate,'configuration':{'count':count,'horizon_days':horizon_days,'tail_scale':tail_scale,'grid':grid,'final':final,'closed_daily':closed,'start_day':days[0],'end_day':days[-1]},
...
def main():
    ...
    if args.phase=='train':
        ...
        jobs=[(k,d,args.iterations,args.steps) for k in args.kinds for d in cutoffs];worker=train_job
    else:
    ...
    dump(ROOT/f'artifacts/execution-{args.phase}.json',{'command':'python3 '+' '.join(sys.argv),'exit_code':0,'runtime_seconds':time.perf_counter()-begin,
