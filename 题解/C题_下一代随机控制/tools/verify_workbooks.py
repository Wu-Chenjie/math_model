...
selection=json.loads((ROOT/'artifacts/export-selection.json').read_text())
source_paths=['artifacts/workbook-payload.json','artifacts/export-selection.json','artifacts/model-adoption.json','artifacts/data.npz']
for kind in ('q1','q2','q3','q4_2','q4_3'):
    for ext in ('npz','json'):
        alias=f'artifacts/{kind}.{ext}'
        source=(f'../C题_下一代基线复现/artifacts/q1.{ext}' if kind=='q1' else f"artifacts/formal/{kind}_{selection['selected'][kind]}.{ext}")
        assert hashlib.sha256((ROOT/alias).read_bytes()).hexdigest()==hashlib.sha256((ROOT/source).read_bytes()).hexdigest()
        source_paths.extend([alias,source])
assert selection['adoption_sha256']==hashlib.sha256((ROOT/'artifacts/model-adoption.json').read_bytes()).hexdigest()
...
(ROOT/'artifacts/workbook-validation.json').write_text(json.dumps({'checks':checks,'source_hashes':{p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in source_paths},'execution':{'command':'bundled-python tools/verify_workbooks.py','exit_code':0,'runtime_seconds':time.perf_counter()-begin}},ensure_ascii=False,indent=2)+'\n')
print(checks)
