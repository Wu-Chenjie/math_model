...
int main(int argc,char**argv){
 ...
 if(count<1||start<1||first<0||last<first||last>57||stress<0||stress>5)return 2;
 std::ofstream f(argv[1]);if(!f)return 2;
 f<<"problem,seed,method,stress,n,cleared,time_s,avg_s,move_m,measures,switches,miss,fallbacks,dp_calls,dp_actions,dp_fallbacks,dp_expanded,dp_budget_hits,aux_calls,aux_positive,joint_steps,runtime_s,certificate\n";
 for(int prob:{3,4})for(int seed=start;seed<start+count;++seed){auto environment=make_case(prob,seed,stress);
  for(int method=first;method<=last;++method){auto options=benchmark_options(method,prob);Sim sim(environment,seed,stress==2?2:stress==4?3:stress==5?4:0);Policy bot(sim,prob,options);
