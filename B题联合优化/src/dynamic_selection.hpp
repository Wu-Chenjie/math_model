...
inline Choice plan(const geo::Poly&p,geo::Pt now,geo::Pt first,double angle,const std::vector<Event>&history,int problem,int depth,bool tuned,int limit,bool retreat=false,int samples=32,bool clear_neighborhood=false){
