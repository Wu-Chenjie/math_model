...
int main(int argc,char**argv){try{
 ...
 std::string strategy=argc>5?argv[5]:"joint";if(strategy!="joint"&&strategy!="baseline"&&strategy!="stage1"&&strategy!="stage2")throw std::runtime_error("strategy must be joint, stage1, stage2 or baseline");
 Options o=joint_configuration(strategy=="joint"?56:strategy=="stage2"?39:strategy=="stage1"?38:0,prob);if(argc>2)o.early=atof(argv[2]);if(argc>3)o.eta=atof(argv[3]);
 ...
 std::cerr<<"strategy="<<strategy<<" planner_depth="<<o.dynamic_depth<<" dp_calls="<<bot.dynamic_plans<<" dp_actions="<<bot.dynamic_actions<<" dp_fallbacks="<<bot.dynamic_fallbacks<<" budget_hits="<<bot.dp_budget_hits<<"\n";
