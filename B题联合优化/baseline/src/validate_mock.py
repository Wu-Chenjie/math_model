...
def main():
    p=argparse.ArgumentParser();p.add_argument('--count',type=int,default=30);p.add_argument('--early',type=float,default=35);p.add_argument('--planner-depth',type=int,choices=[0,1,2,3],default=2)
    a=p.parse_args();rows=[]
    for problem in [3,4]:
        for seed in range(1310001,1310001+a.count):
            client=KernelClient(problem,seed)
            out=drive(problem,client,a.early,planner_depth=a.planner_depth)
            assert out['cleared']==client.session.case.total
            rows.append(dict(layer='original_kernel',problem=problem,seed=seed,planner_depth=out['planner_depth'],**client.session.statistics(out['runtime_s']),certificate=out['certificate']))
        for seed in [1320001,1320002,1320003]:
            ...
            file=ROOT/'results'/f'LOCAL_HTTP_D{a.planner_depth}_P{problem}_{seed}.jsonl'
            try:
    ...
    (ROOT/'results'/f'original_mock_depth{a.planner_depth}.json').write_text(json.dumps(rows,ensure_ascii=False,indent=2),encoding='utf-8')
